...
$objRouter->group(["middleware" => ["check.header"]], function (Router $objRouter) {
    ...
    $objRouter->group(["namespace" => "Office", "prefix" => "office/support", "middleware" => "auth:api"], function (Router $objRouter) {
        ...
        $objRouter->get("correspondence", "Correspondence@getCorrespondence");
        ...
        $objRouter->group(["prefix" => "correspondence"], function (Router $objRouter) {
            ...
            $objRouter->get("/{correspondence_uuid}/messages", "Correspondence@getCorrespondenceMessages");
            ...
            $objRouter->delete("/{correspondence_uuid}/remove","Correspondence@deleteCorrespondence");

            $objRouter->post("/user","Correspondence@addUserToCorrespondence");
            $objRouter->delete("/user","Correspondence@removeUserFromCorrespondence");

            $objRouter->post("/email","Correspondence@addEmailToCorrespondence");
            $objRouter->delete("/email","Correspondence@removeEmailFromCorrespondence");

            $objRouter->post("/group","Correspondence@addGroupToCorrespondence");
            $objRouter->delete("/group","Correspondence@removeGroupFromCorrespondence");
    ...
    $objRouter->group(["prefix" => "office"], function (Router $objRouter) {
        ...
        $objRouter->group(["middleware" => "auth:api"], function (Router $objRouter) {
            $objRouter->group(["prefix" => "user", "namespace" => "Core"], function (Router $objRouter) {
                $objRouter->post("", "User@createAccountOffice");
                ...
                $objRouter->get("{user}/w9", "User@downloadW9Form");
                $objRouter->post("{user}/w9", "User@uploadW9Form");
                $objRouter->post("{user}/w9/status", "User@setW9Status");
            });
            ...
            $objRouter->group(["prefix" => "soundblock", "namespace" => "Office"], function (Router $objRouter) {
                ...
                $objRouter->get("deployments/takedowns", "Deployment@getTakedowns");
                $objRouter->patch("deployment/takedown", "Deployment@takedown");
                $objRouter->post("deployment/files/reupload", "Deployment@reUploadFiles");
                ...
                $objRouter->get("account_plans_types", "AccountPlan@indexTypes");
                ...
                $objRouter->get("tools/reports", "Report@getReports");
                $objRouter->post("tools/reports/upload", "Report@storeReports");
                /** Soundblock Guidance  */
                $objRouter->group(["prefix" => "guidance"], function (Router $objRouter) {
                $objRouter->post("/","Guidance@createGuidanceInstruction");
                $objRouter->get("/","Guidance@listAllGuidanceInstructions");
                $objRouter->get("/{guide_ref}","Guidance@getAGuidanceInstruction");
                $objRouter->patch("/{guide_ref}","Guidance@editGuidanceInstruction");
                $objRouter->delete("/{guide_ref}","Guidance@deleteGuidanceInstruction");
                $objRouter->post("/feedback/reply","Guidance@replyToFeedback");
                $objRouter->get("/feedback/{feedback_uuid}/replies","Guidance@getFeedbackReplies");
                $objRouter->get("/feedback/{guide_ref}","Guidance@getGuideFeedbacks");
                $objRouter->get("/guide/refs","Guidance@listGuideRefs");
                $objRouter->get("/guide/titles","Guidance@listGuidetitles");
                });

                /* Payments */
                $objRouter->group(["prefix" => "payments", "namespace" => "Soundblock"], function (Router $objRouter) {
                    $objRouter->get("/withdrawal", "PaymentBalance@getWithdrawal");
                    $objRouter->patch("/withdrawal", "PaymentBalance@updateWithdrawalStatus");
                });
            });
        ...
        $objRouter->group(["prefix" => "bootloader", "middleware" => "auth:api", "namespace" => "Office"], function (Router $objRouter) {
            $objRouter->get("user/groups", "Bootloader@getAuthUserGroup");
            $objRouter->get("/", "Bootloader@index");
        });
        ...
        $objRouter->group(["prefix" => "music", "namespace" => "Office\Music", "middleware" => "auth:api"], function (Router $objRouter) {
            ...
            $objRouter->group(["prefix" => "/artist"], function (Router $objRouter) {
                ...
                $objRouter->post("/{artist}", "Artists@update");
            });
            ...
            $objRouter->group(["prefix" => "project"], function (Router $objRouter) {
                ...
                $objRouter->post("/", "Projects@create");
