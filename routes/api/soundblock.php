...
$objRouter->group(["middleware" => ["check.header"]], function (Router $objRouter) {
    ...
    $objRouter->group(["prefix" => "soundblock", "middleware" => "auth:api"], function (Router $objRouter) {
        ...
        $objRouter->group(["namespace" => "Soundblock"], function (Router $objRouter) {
            ...
            $objRouter->get("platforms/reports/status", "Reports@getPlatformReportStatus");
        ...
        $objRouter->group(["namespace" => "Core"], function (Router $objRouter) {
            $objRouter->patch("w9", "User@userUploadedForm");
            $objRouter->patch("w9/notapplicable", "User@userFormNotApplicable");
        });
        ...
        $objRouter->group(["namespace" => "Soundblock"], function (Router $objRouter) {
            ...
            $objRouter->group(["prefix" => "account"], function (Router $objRouter) {
                $objRouter->group(["prefix" => "database"], function (Router $objRouter) {
                    ...
                    $objRouter->group(["prefix" => "contributors"], function (Router $objRouter) {
                        $objRouter->get("", "Contributor@index");
                        $objRouter->post("", "Contributor@store");
                        $objRouter->patch("", "Contributor@update");
                        $objRouter->delete("", "Contributor@delete");
                    });
                });
            });
            ...
            $objRouter->group(["prefix" => "account"], function (Router $objRouter) {
                ...
                $objRouter->get("/{account}/plan/past_due", "AccountPlan@getDataForPastDueAccount");
                $objRouter->post("/{account}/plan/past_due", "AccountPlan@chargePastDueAccountPlan");
                ...
                $objRouter->get("/{account}/teams/users", "Account@getTeamsUsers");
            });
            ...
            $objRouter->group(["prefix" => "reports"], function (Router $objRouter) {
                ...
                $objRouter->options("/usage", "Reports@getAvailableUsageData");

                $objRouter->get("/revenue/payment", "Reports@getRevenueReportPayment");
                $objRouter->options("/revenue/payment", "Reports@getAvailableMetaDataForUser");
            });

            $objRouter->group(["prefix" => "payment"], function (Router $objRouter) {
                $objRouter->get("balance/current", "PaymentBalance@getUserBalance");
                $objRouter->get("balance/history", "PaymentBalance@getUserBalanceHistory");
                $objRouter->post("withdrawal", "PaymentBalance@withdrawal");
            });
        });
        ...
        $objRouter->group(["namespace" => "Soundblock"], function (Router $objRouter) {
            ...
            $objRouter->delete("draft/{draft_uuid}", "ProjectDraft@delete");

            $objRouter->options("projects", "Project@indexOptions");
            ...
            $objRouter->group(["prefix" => "project"], function (Router $objRouter) {
                ...
                $objRouter->group(["prefix" => "collection"], function (Router $objRouter) {
                    ...
                    $objRouter->patch("file/path", "Collection@editFilePath");
                    $objRouter->delete("files", "Collection@deleteFiles");
                    $objRouter->post("tracks/swap", "Collection@swapTracks");
                    $objRouter->post("tracks/reorder", "Collection@reorderTracks");
                    $objRouter->get("{collection}/download", "Collection@zipFiles");
                    $objRouter->get("{collection}/directories/files", "Collection@getFilesDirectories");
...
//                        $objRouter->delete("directory", "Collection@deleteDirectory");
                ...
                $objRouter->patch("{project}/deployment/{deployment}/redeploy", "Deployment@redeploy");
                $objRouter->patch("{project}/deployment/{deployment}/takedown", "Deployment@takedown");
        ...
        $objRouter->group(["namespace" => "Soundblock"], function (Router $objRouter) {
            ...
            $objRouter->post("/invite/account/email/{invite}", "AccountInvite@acceptInviteEmail");
            $objRouter->delete("/invite/account/{account}", "AccountInvite@rejectInvite");
            $objRouter->delete("/invite/account/email/{invite}", "AccountInvite@rejectInviteEmail");
        });
        ...
        /**Guidance API */
        $objRouter->group(["namespace" => "Soundblock", "prefix" => "guidance"], function (Router $objRouter) {
            $objRouter->get("/{guide_ref}","Guidance@getGuidance");
            $objRouter->post("/feedback","Guidance@storeGuidanceFeedback");
            $objRouter->post("/rating","Guidance@rateGuide");
        });
...
$objRouter->group(["prefix" => "soundblock", "namespace" => "Soundblock"], function (Router $objRouter) {
    ...
    //conference related api starts
    $objRouter->group(["prefix" => "conference","middleware" => "auth:api"], function (Router $objRouter) {
        $objRouter->post("create-room", "Conference@createRoom")->name('conference.createRoom');
        $objRouter->post("connect", "Conference@connectRoom")->name('conference.connectRoom');
        $objRouter->post("destroy", "Conference@destroyRoom")->name('conference.destroyRoom');
        $objRouter->post("disconnect", "Conference@disconnectRoom")->name('conference.disconnectRoom');
        $objRouter->post("generate-token", "Conference@generateToken")->name('conference.generateToken');
        $objRouter->get("room", "Conference@roomDetails")->name('conference.roomDetails');
        $objRouter->get("room/recordings", "Conference@getRoomRecordings")->name('conference.getRoomRecordings');
        $objRouter->get("rooms", "Conference@getRooms")->name('conference.getRooms');
        $objRouter->get("room/participants", "Conference@getRoomParticipants");
        $objRouter->get("room/participant", "Conference@getRoomParticipant");
        $objRouter->delete("room/participant", "Conference@disconnectRoomParticipant");
        $objRouter->post("room/participant", "Conference@storeRoomParticipant");
        $objRouter->post("ping", "Conference@pingConference");
        $objRouter->delete("room/participants/inactive","conference@removeInactiveParticipants");
        $objRouter->get("/participant/media/{uuid}","Conference@getParticipantMediaUrl");
        $objRouter->get("/room/{room_sid}","Conference@getRoomBySid");

    });
    //conference related api ends
