...
$objRouter->group(["middleware" => ["check.header", "auth:api"]], function (Router $objRouter) {
    ...
    $objRouter->group(["prefix" => "user"], function (Router $objRouter) {
        ...
        $objRouter->group(["prefix" => "profile", "namespace" => "Account"], function (Router $objRouter) {
            ...
            $objRouter->group(["prefix" => "paymethod"], function (Router $objRouter) {
                $objRouter->get("/", "Profile@getPayMethods");
                $objRouter->post("/", "Profile@storePayMethod");
                $objRouter->patch("/", "Profile@updatePayMethod");
                $objRouter->delete("/", "Profile@deletePayMethod");
            });
