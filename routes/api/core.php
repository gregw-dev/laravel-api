...
$objRouter->group(["middleware" => ["check.header", "auth:api"]], function (Router $objRouter) {
    $objRouter->group(["prefix" => "core", "namespace" => "Core"], function (Router $objRouter) {
        ...
        $objRouter->group(["prefix" => "server"], function (Router $objRouter) {
            $objRouter->get("cache/flush", "Server@flushCache");
            $objRouter->get("reports/revenue", "Server@runRevenueReports");
            $objRouter->get("reports/upc", "Server@updateUpcRecords");
            $objRouter->get("reports/unmatched", "Server@processUnmatchedRecords");
            $objRouter->get("allmusic/extract", "Server@allMusicScript");
            $objRouter->get("delete/prod/files", "Server@deleteProdLocalFiles");
        });
...
$objRouter->group(["namespace" => "Core", "middleware" => ["check.header"]], function (Router $objRouter) {
    $objRouter->group(["prefix" => "core"], function (Router $objRouter) {
        $objRouter->group(["prefix" => "correspondence"], function (Router $objRouter) {
            ...
            $objRouter->get("/", "Correspondence@getCorrespondence");
        });
...
$objRouter->group(["middleware" => ["check.header", "auth:api"], "namespace" => "Core\Auth"], function (Router $objRouter) {
    $objRouter->post("auth/signinas", "AuthController@signInAs")->name("sign.in.as");
});
    ...
    $objRouter->group(["prefix" => "core/auth", "namespace" => "Core\Auth"], function (Router $objRouter) {
        ...
        $objRouter->group(["prefix" => "access", "middleware" => ["auth:api"]], function (Router $objRouter) {
            $objRouter->group(["prefix" => "group"], function (Router $objRouter) {
                ...
                $objRouter->get("/{group_uuid}/users", "AuthGroup@getAuthGroupUsers");
                /* Group User */
...
//            $objRouter->delete("permissions-user", "AuthPermission@deletePermissionsInUser");
            ...
            $objRouter->get("/user/permissions", "AuthPermission@getUserPermisssionWithGroups");
        });
    });
});


/* Correspondence Sent By Mailgun Route */
$objRouter->group(["namespace" => "Core"], function (Router $objRouter) {
$objRouter->post("/core/correspondence/ingest","Correspondence@ingestCorrespondence");
});
