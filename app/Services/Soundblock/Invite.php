...
class Invite implements InviteContract {
    ...
    public function find(string $invite){
        return ($this->inviteRepository->find($invite));
    }
    ...
    public function useInvite(Invites $invite, array $userData): User {
        ...
        $boolResult = $this->emailService->canCreateEmail($invite->invite_email);
     ...
     * @throws \Exception
     */
    public function useInviteWithoutCreatingUser(Invites $invite, User $objUser): User {
        $objInvitable = $invite->invitable;
        ...
        if (is_array($invite->invite_permissions)) {
            $this->authPermissionService->updateProjectAndAccountGroupPermissions($invite->invite_permissions, $objUser, $objProject, Client::app());
        }

        $this->commonService->acceptInvite($objProject->account, $objUser);
        $invite->update(["flag_used" => true]);
    ...
    public function rejectEmailInvite(Invites $objInvite, User $objUser){
        $objInvite->delete();
        $objInvitable = $objInvite->invitable;
        $objProject = $objInvitable->project;
        $objAccount = $objProject->account;
