...
class Platform {
     ...
     * @param string|null $category
     * @param false $allowRedeploy
     * @return \Illuminate\Database\Eloquent\Builder[]|\Illuminate\Database\Eloquent\Collection
     *
     * this will returns all the the projects that are not deployed,
     * in case if you want to allow redeploy, you might need to pass $allowRedeploy as true
     * if you set allow redeploy to ture, you will receive all platform with deployment status other than
     *  ['Deployed','Pending']
     */
    public function findNotDeployedForProject(ProjectModel $project, ?string $category = null, bool $allowRedeploy = false) {
        $query = $project->deployments();
        if($allowRedeploy){
            $query = $query->whereIn('deployment_status',['Deployed','Pending']);
        }
        $arrDeployed = $query->pluck("platform_id")->toArray();
