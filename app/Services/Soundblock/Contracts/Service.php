...
class Service implements SmartContracts {
    ...
    public function calculateUserPayoutBetweenDates(Contract $objContract, string $dateStarts, string $dateEnds, float $revenue, float $revenueUsd){
        /* Get general data */
        $objCurrentContractUsers = $objContract->users()->where("contract_version", $objContract->contract_version)->get();
        $arrUsers=[];
        /* Check if contract didn't change during report date month and return current contract users revenue */
        if (
            $objContract->history()->where("history_event",  "active")->get()->count() === 1 || // there are only one active version
            (Carbon::parse($objContract->stamp_updated_at)->setTime(0,0,0)->lessThanOrEqualTo(Carbon::parse($dateStarts))) // currently activate version is activated before report dates
        ) {
            // currently active version is the only version applicable for this date range
            foreach ($objCurrentContractUsers as $objUser) {
                $arrUsers[$objUser->user_id] = [
                    "user_id"       => $objUser->user_id,
                    "user_uuid"     => $objUser->user_uuid,
                    "revenue"       => ($objUser->pivot->user_payout / 100) * $revenue,
                    "revenue_usd"   => ($objUser->pivot->user_payout / 100) * $revenueUsd,
                ];
            }
        } else {
            $floatRevenuePerDay = $revenue /  Carbon::parse($dateStarts)->diffInDays(Carbon::parse($dateEnds));
            $floatRevenuePerDayUsd = $revenueUsd /  Carbon::parse($dateStarts)->diffInDays(Carbon::parse($dateEnds));
            $arrContractVersionDates = $this->getAllContractVersionWithinDate($objContract, $dateStarts, $dateEnds);

            foreach ($arrContractVersionDates as $intContractVersion => $intContractDuration) {
                $objCurrentContractUsers = $objContract->users()->where("contract_version", $intContractVersion)->get();

                foreach ($objCurrentContractUsers as $objUser) {
                    $floatUserRevenue = ($objUser->pivot->user_payout / 100) * ($floatRevenuePerDay * $intContractDuration);
                    $floatUserRevenueUsd = ($objUser->pivot->user_payout / 100) * ($floatRevenuePerDayUsd * $intContractDuration);
                    $arrUsers[$objUser->user_id] = [
                        "user_id"       => $objUser->user_id,
                        "user_uuid"     => $objUser->user_uuid,
                        "revenue"       => $floatUserRevenue + ($arrUsers[$objUser->user_id]["revenue"]??0),
                        "revenue_usd"   => $floatUserRevenueUsd +($arrUsers[$objUser->user_id]["revenue_usd"]??0),
                    ];
                }
            }
        }
    ...
    private function getAllContractVersionWithinDate(Contract $objContract, $strDateStarts, $strDateEnds)
    {
        $objContractHistories=$objContract->history()
            ->where(BaseModel::CREATED_AT,  "<",$strDateEnds)
            ->where("history_event",  "active")
            ->orderByDesc(BaseModel::CREATED_AT)
            ->get();

        $arrVersionDuration=[];
        $intRemainingDays = Carbon::parse($strDateStarts)->diffInDays(Carbon::parse($strDateEnds));
        $objCurrentDateEnds = Carbon::parse($strDateEnds);
        foreach ($objContractHistories as $objContractHistory) {
            $objCurrentDateStarts=$objContractHistory->{BaseModel::CREATED_AT};
            if ($objCurrentDateStarts->lessThan(Carbon::parse($strDateStarts))){
                $objCurrentDateStarts = Carbon::parse($strDateStarts);
            }

            $intCurrentDuration = $objCurrentDateEnds->diffInDays($objCurrentDateStarts);
            $intCurrentVersion = $objContractHistory->contract_state["contract_version"];

            $arrVersionDuration[$intCurrentVersion] = ($arrVersionDuration[$intCurrentVersion]??0)+ $intCurrentDuration;

            $intRemainingDays-= $intCurrentDuration;
            $objCurrentDateEnds= $objCurrentDateStarts;
            if ($intRemainingDays < 1){
                break;
            }
        }
        // add remaining days to last activeDays
        if ($intRemainingDays > 0){
            $arrVersionDuration[$intCurrentVersion]   += $intRemainingDays;
        }
        return $arrVersionDuration;
    }
    ...
    public function update(Contract $objContract, array $arrParams): Contract {
        try {
            ...
            dispatch(new ContractLedgerJob($objContract, $strUpdateEvent, $this->arrUserMeta))->onQueue("ledger");
    ...
    public function acceptContract(Contract $contract, User $user, bool $flagNotifyUser = true): Contract {
        $objOwnAccount = $user->userAccounts()->find($contract->project->account_id);
        $objAccount = $user->accounts()->find($contract->project->account_id);

        if (is_null($objOwnAccount) && $objAccount->pivot->flag_accepted == false) {
        ...
        dispatch(new ContractLedgerJob($contract, ContractLedgerService::USER_ACCEPT_ACTION, $this->arrUserMeta, $user))->onQueue("ledger");
        ...
        foreach ($arrContractUsers as $objUser) {
            if (($objUser->user_uuid != $user->user_uuid) && ($objUser->user_uuid != $contract->account->user->user_uuid)) {
                Mail::to($objUser->primary_email->user_auth_email)->send(new Accepted($contract, Client::app(), false, $user->name));
            }
        }
        ...
        if (!$hasNotAcceptedUser) {
            ...
            dispatch(new ContractLedgerJob($contract, ContractLedgerService::ACTIVATE_CONTRACT, $this->arrUserMeta))->onQueue("ledger");
    ...
    public function rejectContract(Contract $contract, User $user): Contract {
        ...
        event(new ContractChanges($contract));
        dispatch(new ContractLedgerJob($contract, ContractLedgerService::USER_REJECT_ACTION, $this->arrUserMeta, $user))->onQueue("ledger");
    ...
    public function cancelContract(Contract $contract, User $objUser): Contract {
        ...
        dispatch(new ContractLedgerJob($contract, ContractLedgerService::CANCEL_CONTRACT, $this->arrUserMeta))->onQueue("ledger");
