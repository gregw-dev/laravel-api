...
abstract class BaseLedger {
     ...
     * @throws \Exception
     */
    public function createDocument(BaseModel $model, array $arrData, string $strEvent, array $arrUserMeta): LedgerModel {
        $objLedger = $this->ledgerRepo->create([
            ...
            "remote_addr" => $arrUserMeta["remote_addr"] ?? request()->getClientIp(),
            "remote_host" => $arrUserMeta["remote_host"] ?? gethostbyaddr(request()->getClientIp()),
            "remote_agent" => $arrUserMeta["remote_agent"] ?? request()->server("HTTP_USER_AGENT")
        ]);
        ...
        $this->ledgerHistoryRepo->createHistory($objLedger, $arrInsertedData, $strEvent);
