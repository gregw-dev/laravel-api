...
class Tracks implements TracksContract {
    ...
    public function uploadProjectTrackFile(string $project, string $track, $file): bool{
        ...
        $objTrack = $objProject->tracks()->where("music_projects_tracks.track_uuid", $track)->orWhere("music_projects_tracks.track_id", $track)->first();
        ...
        $strLocalFilePath = Storage::disk("local")->putFileAs(Music::project_track_path($objProject, $objTrack->track_uuid), $file, $objTrack->track_name . "." . $fileExt);
        ...
        try {
            $strLocalFilePath = Storage::disk("local")->path($strLocalFilePath);
            $media = MediaFile::open($strLocalFilePath);

            if ($media->isAudio()) {
                $audio = $media->getAudio();
                $duration = intval($audio->getLength());
                $this->objProjectTracksRepo->updateDuration($objTrack, intval($duration));
            } else {
                throw new Exception("Can't set duration, file is not audio.", 400);
            }
        } catch (\Throwable $th) {
            Storage::disk("local")->delete($strLocalFilePath);
            throw new Exception("Set duration file exception.", 400);
        }
