...
class Projects implements ProjectsContract {
    ...
    public function createProject(array $arrParams)
    {

        $objArtist  = $this->objArtistRepository->find($arrParams["artist"]);
        if (!$objArtist) {
            throw new \Exception("Artist Data not found");
        }
        if ($this->checkDuplicate($objArtist->artist_uuid,$arrParams)) {
            throw new \Exception("Project already exists");
        }
        $arrProjectData = [
            "artist_id" => $objArtist->artist_id,
            "artist_uuid" => $objArtist->artist_uuid,
            "project_type" => $arrParams["project_type"],
            "project_date" => $arrParams["project_date"],
            "project_year" => $arrParams["project_year"],
            "project_name" => $arrParams["project_name"],
            "project_label" => $arrParams["project_label"],
            "project_duration" => $arrParams["project_duration"],
            "stamp_epoch" => time(),
            "stamp_date" => now()->toDateString(),
            "stamp_time" => now()->format("H:i:s.u"),
            "stamp_source" => Constant::ARENA_SOURCE,
            "url_allmusic"   => $arrParams["url_allmusic"] ?? "",
            "url_amazon" => $arrParams["url_amazon"] ?? "",
            "url_spotify" => $arrParams["url_spotify"] ?? "",
        ];

        return $this->objProjectRepository->create($arrProjectData);
    }
    ...
    public function checkDuplicate(String $strArtistUuid, Array $arrData) : Bool
    {
        $arrProjectData = [
            "artist_uuid" => $strArtistUuid,
            "project_type" => $arrData["project_type"],
            "project_year" => $arrData["project_year"],
            "project_date" => $arrData["project_date"],
            "project_duration" => $arrData["project_duration"],
        ];
        return $this->objProjectRepository->checkDuplicate($arrProjectData);
    }
}
