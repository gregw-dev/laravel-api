...
class Notifications implements NotificationsContract {
     ...
     * @param int $intCloseIn
     */
    public function notifyGroupPermission(string $strGroup, string $strPermission, App $objApp, string $strName, string $strDescription, ?string $strAction = null, ?string $strUrl = null, bool $bnAutoClose = true, int $intCloseIn = NotificationsContract::CLOSE_TIMEOUT) {
        $objUsers = $this->authGroupService->findAllUsersByGroupAndPermission($strGroup, $strPermission);
