...
class AuthGroup {
    ...
    public function __construct(AuthGroupRepository $groupRepo, UserRepository $userRepo, ProjectRepository $projectRepo,
                                CollectionRepository $colRepo, AuthPermissionRepository $authPermissionRepo) {
    ...
    public function findAllUsersByGroupAndPermission(string $authGroup, string $authPermission){
        $objAuthGroup = $this->findByName($authGroup);
        $objAuthPermission = $this->authPermissionRepo->findByName($authPermission, true);

        return ($this->groupRepo->findAllUsersByGroupAndPermission($objAuthGroup, $objAuthPermission));
    }
