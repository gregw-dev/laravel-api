...
class Zip {
    ...
    public function putArtwork(Project $objProject, $objFile) {
        ...
        $objProject->project_artwork_rand = $artworkCode;
        $objProject->save();

        $artworkPath = Soundblock::project_path($objProject);

        return ($this->saveAvatar("public/" . $artworkPath, $objFile, "artwork.png"));
    }
    ...
    public function moveArtwork(Project $project, string $artwork): string {
        ...
        $project->project_artwork_rand = Util::generateRandomCode();
        $project->save();
        ...
        $destPath = Soundblock::move_project_artwork_path($project);
    ...
    public function moveArtistAvatar(ArtistModel $objArtist): string {
        ...
        $avatarPath = "public/" . Soundblock::artists_avatar_full_path($objArtist);
     ...
     * @throws FileNotFoundException
     */
    public function unzipProject(string $fileName, array $arrFiles, string $strComment, Project $project, User $user, array $arrUserMeta): Collection {
        ...
        $collection = $this->handleProjectFiles($extractPath, $arrFiles, $strComment, $project, $user, $arrUserMeta);
     ...
     * @throws FileNotFoundException
     */
    protected function handleProjectFiles(string $extractPath, array $arrFiles, string $strComment, Project $project, User $user, array $arrUserMeta): Collection {
        $arrStampChanged = [
        ...
        $objTracks = $collection->tracks;
        // store tracks count per volume as [track_volume_number => number of tracks in the volume]
        $arrVolumeTracksCount= [];

        foreach ($collection->tracks as $objTrack) {
        ...
        foreach ($arrFiles as $arrFile) {
            ...
            switch ($arrFile["file_category"]) {
                case Constant::MusicCategory:
                    ...
                    if (isset($arrVolumeTracksCount[$arrFileParam["track_volume_number"]])){
                        $intTrackNum = count($arrVolumeTracksCount[$arrFileParam["track_volume_number"]]) + 1;
                        $arrFileParam["track_number"] = $intTrackNum;
                        $arrVolumeTracksCount[$arrFileParam["track_volume_number"]][] = $intTrackNum;
                    }else{
                        $arrVolumeTracksCount[$arrFileParam["track_volume_number"]][] = 1;
                        $arrFileParam["track_number"] = 1;
                    }

                    $objTracks = $collection->tracks;

                    foreach ($objTracks as $objTrack) {
                        if (($objTrack->track_number == $arrFileParam["track_number"]) && ($objTrack->track_volume_number == $arrFileParam["track_volume_number"])) {
            ...
            $arrFileParam["remote_addr"] = $arrUserMeta["remote_addr"];
            $arrFileParam["remote_host"] = $arrUserMeta["remote_host"];
            $arrFileParam["remote_agent"] = $arrUserMeta["remote_agent"];
            ...
            if ($arrFile["file_category"] === Constant::MusicCategory) {
                ...
                foreach ($arrHistory as $column => $value) {
                    $this->trackHistoryRepo->create([
                        ...
                        "old_value" => null,
                        "new_value" => $value,
                        TrackModel::STAMP_CREATED_BY => $objTrack[TrackModel::STAMP_CREATED_BY],
                        TrackModel::STAMP_UPDATED_BY => $objTrack[TrackModel::STAMP_UPDATED_BY]
                    ]);
                }

                dispatch(new TrackLedger($objTrack, TrackLedgerService::CREATE_EVENT, $arrUserMeta))->onQueue("ledger");
            }

            dispatch(new FileLedger($file, Soundblock::project_file_path($project, $file), $arrUserMeta))->onQueue("ledger");
        ...
        dispatch(new CollectionLedger($collection, CollectionLedgerService::CREATE_EVENT, $arrUserMeta))->onQueue("ledger");
            ...
            case Constant::MusicCategory:
            {
                ...
                try {
                    if (!Storage::disk("local")->exists($filePath)) {
                        Storage::disk("local")->writeStream($filePath, $this->soundblockAdapter->readStream($dest . $physicalName . "." . $extension));
                    }

                    $strPath = Storage::disk("local")->path($filePath);
                ...
                $arrFile["track_language_metadata"]     = $arrParam["track_language_metadata"] ?? null;
                $arrFile["track_language_audio"]        = $arrParam["track_language_audio"] ?? null;
                ...
                $arrFile["track_isrc"]                  = $arrParam["track_isrc"] ?? null;
        ...
        if ($zip->open($this->localAdapter->path($zipPath),ZipArchive::CREATE)) {
            ...
            foreach ($arrFiles as $file) {
                if ($this->soundblockAdapter->exists($file["real_path"])) {
                    ...
                    $zip->addFile($this->localAdapter->path($file["real_path"]), $file["file_path"] . "/" . $file["file_title"] . "." . $file["file_extension"]);
    ...
    public function zipMusic(Collection $collection) {
        ...
        if (count($arrMusic) > 0) {
            ...
            foreach ($arrMusic as $music) {
                ...
                $objFile = $music->file;
                $strRealPath = $strProjectPath . $objFile->file_uuid . "." .
                    pathinfo($objFile->file_name, PATHINFO_EXTENSION);

                if ($this->soundblockAdapter->exists($strRealPath)) {
                    $strTrackPath = Soundblock::deployment_project_track_path($music);

                    if (!$this->officeAdapter->exists("public/" . $strTrackPath)) {
                        $readStream = $this->soundblockAdapter->readStream($strRealPath);
                        $this->officeAdapter->writeStream(
                            "public/" . $strTrackPath,
                            $readStream,
                            ["visibility" => "public"]
                        );
                    ...
                    throw new Exception("Track {$music->track_uuid} does not exist by path: " . $strRealPath);
                }
            }
        } else {
            throw new Exception("Collection tracks not found.");
        }

        try {
            $strZipPath = Soundblock::office_deployment_project_zip_path($collection);

            if ($this->localAdapter->exists($strZipPath)) {
                $this->localAdapter->delete($strZipPath);
            }

            if (!$this->localAdapter->exists("soundblock/" . Soundblock::deployment_project_path($collection->project))) {
                $this->localAdapter->makeDirectory("soundblock/" . Soundblock::deployment_project_path($collection->project));
            }

            $zip = new ZipArchive();
            if ($zip->open($this->localAdapter->path($strZipPath), ZipArchive::CREATE)) {
                if (isset($collection->project->project_artwork_rand)) {
                    $strProjectArtworkPath = Soundblock::zip_project_artwork_path($collection->project);
                    if ($this->soundblockAdapter->exists($strProjectArtworkPath)) {
                        $projectArtworkPath = $this->soundblockAdapter->get($strProjectArtworkPath);
                        $zip->addFromString("Project - " . $collection->project->project_title . " - " . $collection->project->project_uuid . ".png", $projectArtworkPath);
                    }
                }
                ...
                if (!empty($objArtists)) {
                    ...
                    foreach ($objArtists as $objArtist) {
                        $artistAvatarPath = "public/" . Soundblock::artists_avatar_full_path($objArtist);
                        if ($this->soundblockAdapter->exists($artistAvatarPath)) {
                            $artistAvatar = $this->soundblockAdapter->get($artistAvatarPath);
                            $zip->addFromString("Artist - " . $objArtist->artist_name . " - " . $objArtist->artist_uuid . ".png", $artistAvatar);
                        }
                    }
                }
                ...
                if ($res === true) {
                    if ($this->officeAdapter->exists("public/" . $strZipPath)) {
                        $this->officeAdapter->delete("public/" . $strZipPath);
                    }
                    $readStream = $this->localAdapter->readStream($strZipPath);
                    $this->officeAdapter->writeStream("public/" . $strZipPath, $readStream, ["visibility" => "public"]);
                    $this->localAdapter->delete($strZipPath);

                    return ($strZipPath);
                } else {
                    throw new Exception("Issue during images zip file closing.");
                }
            } else {
                throw new Exception("Issue during images zip file opening.");
            }
        } catch (\Exception $exception) {
            throw new Exception("Images zip file failed: " . $exception->getMessage());
        }
    }
