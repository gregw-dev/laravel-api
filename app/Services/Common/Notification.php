...
class Notification {
    ...
    public function findSettingByApp(User $objUser, App $objApp): NotificationSetting {
        $objNotificationSetting = $objUser->notificationSettings()->where("app_id", $objApp->app_id)->first();

        if (is_null($objNotificationSetting)) {
            $objNotificationSetting = $objUser->notificationSettings()->create([
                "row_uuid" => Util::uuid(),
