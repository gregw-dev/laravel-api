...
use App\Mail\Core\Support\TicketOpened;
...
use App\Repositories\{
    ...
    Office\SupportTicketLookup as SupportTicketLookupRepository,
...
class SupportTicket {
    ...
    /** @var SupportTicketLookupRepository */
    private SupportTicketLookupRepository $supportTicketRepo;
    /**
    ...
    public function __construct(SupportTicketRepository $ticketRepo, Support $supportRepo, UserRepository $userRepo,
                                ...
                                SupportTicketMessage $messageService, SupportTicketLookupRepository $supportTicketRepo) {
     ...
     * @return \Illuminate\Database\Eloquent\Collection|\Illuminate\Pagination\LengthAwarePaginator
     */
    public function findAllForOfficeByApps(array $arrParams, array $arrAllowedApps, int $perPage = 10) {
        return ($this->ticketRepo->findAllForOfficeByApps($arrParams, $arrAllowedApps, $perPage));
    }
    ...
    public function checkTicketUserForOffice(array $arrAllowedApps, string $ticket){
        $objTicket = $this->ticketRepo->find($ticket, true);

        if (array_search($objTicket->support->app->app_name, $arrAllowedApps) !== false) {
            return ($objTicket);
        }
    ...
    public function checkTicketDuplicate(string $user_uuid, string $title, string $message){
        return ($this->ticketRepo->checkTicketDuplicate($user_uuid, $title, $message));
    }
    ...
    private function saveTicket(App $app, array $arrParams, User $user, $isOffice = false, ?AuthGroup $objGroup = null): SupportTicketModel {
        ...
        if (is_null($objGroup)) {
            $objGroup = $this->authGroupRepository->findByName("Arena.Support");
        }
    ...
    public function creteOfficeTicket(User $objUserFrom, array $arrParams): SupportTicketModel {
        ...
        if (isset($arrParams["message"])) {
            ...
            $this->messageService->create(true, $arrMsgParams, true, $app);
        }

        if ($objAuthUser->user_id !== $objUserFrom->user_id) {
            Mail::to($objUserFrom->primary_email->user_auth_email)->send(new NewTicket($objTicket,$app));
        }
    ...
    public function creteCoreTicket(User $objUser, App $app, array $arrParams, bool $flagOffice): array {
        ...
        Mail::to($objTicket->user->primary_email->user_auth_email)->send(new TicketOpened($objTicket, $app));
    ...
    public function storeSupportTicketLookup(Object $objTicket, $strMessageId){
    return $this->supportTicketRepo->insert($objTicket, $strMessageId);
    }
}
