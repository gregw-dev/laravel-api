...
use Exception;
use Carbon\Carbon;
use App\Mail\Soundblock\UserW9;
...
class User {
     ...
     */
    public function __construct(UserRepository $userRepo, UserAlias $aliasRepo, UserContactEmailRepository $emailRepo, Email $emailService) {
    ...
    public function find($id, $bnFailure = false) {
        return ($this->userRepo->find($id, $bnFailure));
    }

    public function findForOffice($id, $bnFailure = false){
        $arrUserMeta = [];
        $objUser = $this->userRepo->find($id, $bnFailure);

        $arrUserMeta["user_balance"] = $objUser->current_balance;

        if ($arrUserMeta["user_balance"]) {
            $arrUserMeta["user_balance"]["user_balance"] = floatval($arrUserMeta["user_balance"]["user_balance"]);
        }

        $arrUserMeta["uploaded_w9"] = false;

        if (bucket_storage("office")->exists(SoundblockFileSystemHelper::full_user_w9_form_path($objUser))) {
            $arrUserMeta["uploaded_w9"] = true;
        }

        foreach ($objUser->userAccounts as $objUserAccount) {
            if ($objUserAccount->activePlan) {
                $arrAccountData = [];
                $arrAccountData["Account"] = $objUserAccount->account_name;
                $arrAccountData["Plan"] = $objUserAccount->activePlan->planType->plan_name;
                $arrAccountData["Date"] = Carbon::parse($objUserAccount->stamp_created_at)->format("m-d-Y");

                $arrUserMeta[] = $arrAccountData;
            }
        }

        $arrUserIpInfo = Util::getUserFraudScore($objUser);
        $objUser->user_remote_addr = $arrUserIpInfo["remote_addr"];
        $objUser->fraud_score = $arrUserIpInfo["fraud_score"];

        return ([$objUser, $arrUserMeta]);
    }
     ...
     * @throws Exception
     */
    public function createAccountOffice(array $arrParams) {
        $arrParams["name_first"] = $arrParams["name"];
        unset($arrParams["name"]);
        $arrParams["user_password"] = $arrParams["password"];
        $arrParams[UserModel::STAMP_CREATED_BY] = auth()->user()->id;
        $arrParams[UserModel::STAMP_UPDATED_BY] = auth()->user()->id;
        $arrParams["flag_changepassword"] = 1;

        if(!$this->emailService->canCreateEmail($arrParams["email"])){
            throw new Exception("The email has already been taken.");
        }

        $objUser = $this->create($arrParams);

        $this->createEmail($objUser, $arrParams["email"]);

        $alias = strtolower(substr($objUser->name_first,0,1).$objUser->name_last);
        $this->createAlias($objUser, $alias);
    ...
    public function create(array $arrParams): UserModel {
        ...
        if (isset($arrParams["flag_changepassword"])){
            $arrUser["flag_changepassword"] = $arrParams["flag_changepassword"];
        }
    ...
    public function uploadW9(array $arrData){
        $objUser = $this->userRepo->find($arrData["user"]);
        if (env("APP_ENV") == "local") {
            $officeAdapter = Storage::disk("local");
        } else {
            $officeAdapter = bucket_storage("office");
        }

        $uploadPath = SoundblockFileSystemHelper::user_w9_form_path($objUser);
        $officeAdapter->putFileAs($uploadPath, $arrData["form"], "w9.pdf");

        $this->updateW9($objUser->user_uuid, "true");

        Mail::to($objUser->primary_email->user_auth_email)->send(new UserW9());

        return (true);
    }

    public function updateW9(string $strUserUuid, string $strStatus){
        $objUser = $this->userRepo->find($strUserUuid, true);
        $objUser->update(["flag_w9" => $strStatus]);
        $objUser->refresh();
