...
class AuthPermission extends BaseRepository {
    ...
    public function findAll(array $arrParams, int $per_page = 10){
        ...
        $objAuthPermissions = $query->paginate($per_page);

        return ([$objAuthPermissions, $availableMetaData]);
    }
    ...
    public function findAllByGroup(AuthGroup $objGroup){
        return (
            $objGroup->permissions()->select("core_auth_permissions.*", "core_auth_permissions_groups.permission_value")->get()
        );
    }
}
