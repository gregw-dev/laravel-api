...
class AuthGroup extends BaseRepository {
    ...
    public function findAllUsersByGroupAndPermission(AuthGroupModel $objAuthGroup, AuthPermission $objAuthPermission){
        $objUsers = $objAuthPermission->pusers()
            ->where("core_auth_permissions_groups_users.group_id", $objAuthGroup->group_id)
            ->where("core_auth_permissions_groups_users.permission_value", 1)
            ->get();
