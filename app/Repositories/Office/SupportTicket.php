...
class SupportTicket extends BaseRepository {
     ...
     * @return \Illuminate\Database\Eloquent\Collection|LengthAwarePaginator
     */
    public function findAllForOfficeByApps(array $arrParams, array $arrAllowedApps, ?int $perPage = null){
        $query = $this->model->join("support", "support_tickets.support_id", "=", "support.support_id")
            ->join("core_apps", "support.app_id", "=", "core_apps.app_id")
            ->with(["support", "support.app"])
            ->whereIn("core_apps.app_name", $arrAllowedApps)
            ->select("support_tickets.*");
        ...
        return (
            $this->model->where("user_uuid", $user_uuid)
                ...
                ->whereHas("messages", function ($query) use ($message) {
                    $query->whereRaw("lower(message_text) = (?)", Util::lowerLabel($message));
                })
                ->whereDate("stamp_created_at", "=", Carbon::today()->toDateString())
            ->exists()
        );
    }
    ...
    protected function applyFilter(Builder $query, array $arrParams, ?int $perPage = null) {
        $query = $query->orderBy("support_tickets.stamp_updated_at", "desc");
        ...
        if(isset($arrParams["flag_status"])) {
            if ($arrParams["flag_status"] != "All") {
                if ($arrParams["flag_status"] == "Open") {
                    $query = $query->whereRaw("lower(support_tickets.flag_status) != (?)", Util::lowerLabel("Closed"));
                } else {
