...
class Artist extends BaseRepository {
    ...
    public function create(array $arrParams): ArtistModel {
        $arrArtistData = [
            "arena_id"      => "",
            "artist_name"   => $arrParams["artist_name"],
    ...
    public function autoComplete(string $name, int $perPage = 10) {
        return $this->model->whereRaw("lower(artist_name) like (?)", "%" . Util::lowerLabel($name) . "%")->get(["artist_uuid","artist_name", "url_allmusic"]);
    }

    public function checkDuplicate(Array $arrParams) : Bool
    {
        $arrArtistData = [
            "artist_name"   => $arrParams["artist_name"],
            "artist_born"   => $arrParams["born"],
        ...
        return $this->model->where($arrArtistData)->exists();
    }
}
