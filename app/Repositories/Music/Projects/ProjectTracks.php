...
class ProjectTracks extends BaseRepository {
    ...
    public function attachGenres($objTrack, array $genres){
        foreach ($genres as $genre) {
            ...
            $objTrack->genres()->attach($objGenre->row_id,[
                "row_uuid" => Util::uuid(),
                "genre_uuid" => $objGenre->row_uuid,
    ...
    public function attachMoods($objTrack, array $moods){
        foreach ($moods as $mood) {
            ...
            $objTrack->moods()->attach($objMood->row_id, [
                "row_uuid" => Util::uuid(),
                "mood_uuid" => $objMood->row_uuid,
                "track_id" => $objTrack->track_id,
                "track_uuid" => $objTrack->track_uuid,
                "stamp_epoch" => time(),
                "stamp_date" => date("Y-m-d"),
    ...
    public function attachStyles($objTrack, array $styles){
        foreach ($styles as $style) {
            ...
            $objTrack->styles()->attach($objStyle->row_id, [
                "row_uuid" => Util::uuid(),
                "track_id" => $objTrack->track_id,
                "track_uuid" => $objTrack->track_uuid,
                "style_uuid" => $objStyle->row_uuid,
                "stamp_epoch" => time(),
                "stamp_date" => date("Y-m-d"),
                "stamp_time" => date("G:i:s"),
            ]);
    ...
    public function attachThemes($objTrack, array $themes){
        foreach ($themes as $theme) {
            ...
            $objTrack->themes()->attach($objTheme->row_id, [
                "row_uuid" => Util::uuid(),
                "theme_uuid" => $objTheme->row_uuid,
                "track_id" => $objTrack->track_id,
                "track_uuid" => $objTrack->track_uuid,
                "stamp_epoch" => time(),
                "stamp_date" => date("Y-m-d"),
