...
class Deployment extends BaseRepository {
    ...
    public function findAllWithRelationsAndStatus(array $arrRelations, array $arrStatus, int $per_page, array $arrData, array $arrSort = [], bool $bnGroupByCollection = false) {
        ...
        if (isset($arrData["date_from"], $arrData["date_to"])) {
            $dateFrom = Carbon::create($arrData["date_from"])->startOfDay();
            $dateTo = Carbon::create($arrData["date_to"])->endOfDay();
            $query = $query->whereBetween("soundblock_projects.project_date", [$dateFrom, $dateTo]);
        }
        ...
        if (isset($arrSort["sort_project"])) {
            ...
            $query->orderBy("soundblock_projects.project_date", "asc");
        }
    ...
    public function findAllPending(){
        return ($this->model->where("deployment_status", "Pending")->count());
    }
    ...
    public function canDeployOnPlatform(Collection $collection, Platform $platform) {
        return (!$this->model->where("collection_id", $collection->collection_id)
            ...
            ->WhereIn('deployment_status',['Pending','Deployed'])->exists());
    }
