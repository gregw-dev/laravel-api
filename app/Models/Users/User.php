...
use App\Models\{Common\QueueJob,
    ...
    Users\Accounting\AccountingPayMethods,
    ...
    Soundblock\Payments\MusicUserBalancePayment
};
...
class User extends Authenticatable {
     ...
     */
    protected $hidden = [
        "user_id", "user_password", "remember_token", "name_first", "name_middle", "name_last", "user_balance",
        User::DELETED_AT, User::STAMP_DELETED, User::STAMP_DELETED_BY,
        BaseModel::CREATED_AT, BaseModel::UPDATED_AT,
        "pivot", "remote_host", "remote_addr", "remote_agent",
    ];
    protected $appends = [
        "name", "primary_email","avatar",
    ];
    /**
...
//    }
    ...
    public function groupsWithPermissions() {
        return $this->belongsToMany(AuthGroup::class, "core_auth_permissions_groups_users", "user_id", "group_id", "user_id", "group_id")
                     ...
                     ->withTimestamps(BaseModel::CREATED_AT, BaseModel::UPDATED_AT)
                     ->select([
                        "core_auth_groups.group_uuid", "core_auth_groups.auth_uuid", "core_auth_groups.group_name", "core_auth_groups.group_memo", "core_auth_groups.flag_critical",
                         "core_auth_permissions_groups_users.permission_value"
                        ]);
    }
    ...
    public function paymethods() {
        return ($this->hasMany(AccountingPayMethods::class, "user_id", "user_id"));
    }
    ...
    public function balance(){
        return ($this->hasMany(MusicUserBalancePayment::class, "user_id", "user_id"));
    }

    public function getCurrentBalanceAttribute(){
        return ($this->balance()->latest("row_id")->first());
    }
