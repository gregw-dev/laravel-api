...
trait UpdateCollectionFiles
{
    private function updateTrackFileData(TrackModel $objTrack, array $itemFile, SoundblockCollection $objCollection, array $changes)
    {
        ...
        if (array_key_exists("track_release_date", $itemFile)) {
            $itemFile["track_release_date"] = Carbon::parse($itemFile["track_release_date"])->format("Y-m-d");
        }
        ...
        if (array_key_exists("track_language_audio", $itemFile)) {
            $objLang = $this->languagesRepo->find($itemFile["track_language_audio"]);

            if (is_null($objLang)) {
                throw new \Exception("Invalid language.");
            }

            if (optional($objTrack->languageAudio)->data_language !== $objLang->data_language) {
                $changes["Language audio"] = [
                    "Previous value" => optional($objTrack->languageAudio)->data_language,
                    "Changed to" => $objLang->data_language
                ];
                $arrHistory["track_language_audio"] = [
                    "old" => optional($objTrack->languageAudio)->data_language,
                    "new" => $objLang->data_language
                ];
            }

            $objTrack->track_language_audio_id = $objLang->data_id;
            $objTrack->track_language_audio_uuid = $objLang->data_uuid;
        }

        if (array_key_exists("track_language_metadata", $itemFile)) {
            $objLang = $this->languagesRepo->find($itemFile["track_language_metadata"]);

            if (optional($objTrack->languageMetadata)->data_language !== optional($objLang)->data_language) {
                $changes["Language metadata"] = [
                    "Previous value" => optional($objTrack->languageMetadata)->data_language,
                    "Changed to" => optional($objLang)->data_language
                ];
                $arrHistory["track_language_metadata"] = [
                    "old" => optional($objTrack->languageMetadata)->data_language,
                    "new" => optional($objLang)->data_language
                ];
                $objTrack->track_language_metadata_id = optional($objLang)->data_id;
                $objTrack->track_language_metadata_uuid = optional($objLang)->data_uuid;
            }
        }
        ...
        if (array_key_exists("genre_secondary", $itemFile)) {
            if (is_null($itemFile["genre_secondary"])) {
                $objSecondaryGenre = null;
            } else {
            ...
            /* ISRC code is provided and different from existing one */
        if (
            array_key_exists("track_isrc", $itemFile) &&
            !empty($itemFile["track_isrc"]) &&
            (strtolower(str_replace("-", "", $objTrack->track_isrc)) !== strtolower(str_replace("-", "", $itemFile["track_isrc"])))
        ) {
            if (strlen($itemFile["track_isrc"]) == 12 && substr($itemFile["track_isrc"], 0, 2) == "US") {
                $itemFile["track_isrc"] = strtoupper(substr($itemFile["track_isrc"], 0, 2) . "-"
                    . substr($itemFile["track_isrc"], 2, 3) . "-"
                    . substr($itemFile["track_isrc"], 5, 2) . "-"
                    . substr($itemFile["track_isrc"], 7, 5));
            }
             /* Check provided ISRC is not used in  other track / Marked assigned */
            if ($this->trackRepo->findByIsrc($itemFile["track_isrc"]) || ($this->isrcServices->isAssigned($itemFile["track_isrc"]))) {
                throw new \Exception("ISRC code already in use.");
            }

            /* If existing ISRC is marked assigned, release that. */
            if ($this->isrcServices->isAssigned($objTrack->track_isrc)) {
                $objIsrcCode = $this->isrcServices->findByIsrc($objTrack->track_isrc);
                $this->isrcServices->releaseIsrc($objIsrcCode);
            }

            /* if using one of the unused ISRC from us, mark assigned */
            $objNewIsrcCode = $this->isrcServices->findByIsrc($itemFile["track_isrc"]);

            if (is_object($objNewIsrcCode)) {
                $this->isrcServices->useIsrc($objNewIsrcCode);
            }

            /* Change the ISRC and generate changes & history data */
            $changes["Track ISRC"] = [
                "Previous value" => $objTrack->track_isrc,
                "Changed to" => $itemFile["track_isrc"]
            ];
            $arrHistory["track_isrc"] = [
                "old" => $objTrack->track_isrc,
                "new" => $itemFile["track_isrc"]
            ];

            $objTrack->track_isrc = $itemFile["track_isrc"];
        }
