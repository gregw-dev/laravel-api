...
class Accepted extends Mailable implements ShouldQueue {
    ...
    public function build() {
        ...
        $notAcceptedUsers = [];
        $intCurrentCycle = $this->contract->contract_version;
        $contractInvites = $this->contract->contractInvites()->wherePivot("contract_status", "!=", "Accepted")
            ->wherePivot("contract_version", $intCurrentCycle)->get();
        $contractUsers = $this->contract->users()->wherePivot("contract_status", "!=", "Accepted")
            ->wherePivot("contract_version", $intCurrentCycle)->get();

        if (!empty($contractInvites)) {
            foreach ($contractInvites as $contractInvite) {
                $notAcceptedUsers[] = $contractInvite->invite_name;
            }
        }

        if (!empty($contractUsers)) {
            foreach ($contractUsers as $contractUser) {
                $notAcceptedUsers[] = $contractUser->name;
            }
        }
