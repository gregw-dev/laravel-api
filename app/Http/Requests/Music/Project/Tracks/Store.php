<?php

namespace App\Http\Requests\Music\Project\Tracks;

use Illuminate\Foundation\Http\FormRequest;

class Store extends FormRequest
{
    /**
     * Determine if the user is authorized to make this request.
     *
     * @return bool
     */
    public function authorize()
    {
        return true;
    }

    /**
     * Get the validation rules that apply to the request.
     *
     * @return array
     */
    public function rules()
    {
        return [
            "disc_number"  => "required|integer",
            "track_name"   => "required|string|max:255",
            "url_allmusic" => "sometimes|string",
            "url_amazon"   => "sometimes|string",
            "url_spotify"  => "sometimes|string",
            "file"         => "required|file|mimes:wav,mp3",
            "composers"    => "sometimes|array",
            "composers.*"  => "required_with:composers|uuid",
            "features"     => "sometimes|array",
            "features.*"   => "required_with:features|uuid",
            "performers"   => "sometimes|array",
            "performers.*" => "required_with:performers|uuid",
            "genres"       => "sometimes|array",
            "genres.*"     => "required_with:genres|uuid|exists:music_artists_genres,row_uuid",
            "moods"        => "sometimes|array",
            "moods.*"      => "required_with:moods|uuid|exists:music_artists_moods,row_uuid",
            "styles"       => "sometimes|array",
            "styles.*"     => "required_with:styles|uuid|exists:music_artists_styles,row_uuid",
            "themes"       => "sometimes|array",
            "themes.*"     => "required_with:themes|uuid|exists:music_artists_themes,row_uuid",
        ];
    }
}
