...
class EditFilesInCollection extends FormRequest {
    ...
    public function rules() {
        return ([
            ...
            "files.*.track_isrc"                    => "sometimes|alpha_dash|min:12|max:15",
            ...
            "files.*.track_language_metadata"       => "sometimes|string|exists:soundblock_data_languages,data_uuid",
            "files.*.track_language_audio"          => "required_if:files.*.flag_track_instrumental,0|string|max:255",
            "files.*.track_volume_number"           => ["sometimes", "integer", "min:1", new TrackVolumeNumber($this->all())],
            "files.*.track_release_date"            => ["sometimes", "date", new TrackReleaseDate($this->all())],
