...
class Account extends Controller {
     ...
     */
    public function __construct(Common $commonService, ProjectService $projectService, TeamService $teamService) {
    ...
    public function getUserAccounts(){
        /** @var User $objUser */
        $objUser = Auth::user();
        $objAccounts = $this->commonService->findUserPlans($objUser);

        return ($this->apiReply($objAccounts));
    }
    ...
    public function getTeamsUsers(string $account){
        $objUser = Auth::user();
        $objAccount = $this->commonService->find($account);

        if ($objUser->user_id != $objAccount->user_id) {
            return $this->apiReject(null, "User is not owner of this account.", Response::HTTP_FORBIDDEN);
        }

        $arrData = $this->teamService->getAccountUsers($objAccount);
        $additionalUsersExtraCharge = $this->commonService->getEstimateAdditionalUsersCharge($objAccount);

        return ($this->apiReply($arrData, "", Response::HTTP_OK, ["additional_users_extra_charge" => $additionalUsersExtraCharge]));
    }
