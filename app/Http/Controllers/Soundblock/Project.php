...
use App\Http\Requests\{Office\Project\AddFile,
    ...
    Soundblock\Project\GetProjects,
...
class Project extends Controller {
    ...
    public function index(GetProjects $objRequest) {
        ...
        $arrProject = $this->projectService->findAllByUser($objUser, $objRequest->all());
    ...
    public function indexOptions() {
        /** @var UserModel $objUser */
        $objUser = AuthFacade::user();
        $arrOptions = $this->projectService->findOptionsByUser($objUser);

        return ($this->apiReply($arrOptions, "", 200));
    }
    ...
    public function getArtwork(string $project) {
        ...
        $filePath = Soundblock::upload_project_artwork_path($objProject, $objProject->project_artwork_rand);
    ...
    public function store(CreateProject $objRequest) {
        ...
        dispatch(new ProjectLedger(
            $objProject,
            \App\Services\Soundblock\Ledger\ProjectLedger::CREATE_EVENT,
            [
    ...
    public function update(string $project, UpdateProject $objRequest) {
        ...
        /*
       if ($objProject->flag_music_permanent) {
           return ($this->apiReject("", "You can't edit project while it has active deployment.", Response::HTTP_FORBIDDEN));
      }
         */
