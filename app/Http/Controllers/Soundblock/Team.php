...
class Team extends Controller {
     ...
     * @throws Exception
     */
    public function deleteMember(string $project, string $uuidMember) {
        $boolResult = false;
        ...
        $objUser = $this->userService->find($uuidMember);
        $objProject = $this->projectService->find($project, true);

        if (is_object($objUser)) {
            $boolResult = $this->teamService->delete($objProject, collect([$objUser]), $objGroup);
        } else {
            $objInvite = $this->teamService->getInvite($objProject, $uuidMember);
        ...
        if ($boolResult) {
            ...
            return ($this->apiReply(null, "Team member deleted successfully.", Response::HTTP_OK));
        }

        return ($this->apiReject(null, "Team member is part of the project contract.", Response::HTTP_BAD_REQUEST));
    }
