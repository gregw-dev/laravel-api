...
use App\Http\Requests\{Office\Project\Collection\ZipFile,
    Soundblock\Collection\AddFileToCollection,
    Soundblock\Collection\EditFilePath,
    ...
    Soundblock\Collection\ReorderTracks,
    Soundblock\Collection\SwapTracks,
...
class Collection extends Controller
{
    ...
    public function getTrackCover(string $project, string $file) {
        ...
        if (!bucket_storage("soundblock")->exists($strPath)) {
            $strPath = Soundblock::upload_project_artwork_path($objProject, $objProject->project_artwork_rand);
        }
    ...
    public function getFilesDirectories(string $collection){
        ...
        $strSoundGroup = sprintf("App.Soundblock.Project.%s", $objCollection->project->project_uuid);

        if (!is_authorized($objUser, $strSoundGroup, "App.Soundblock.Project.File.Files.Add", "soundblock")) {
            return ($this->apiReject("", "You don't have required permissions.", Response::HTTP_FORBIDDEN));
        }

        $objDirectories = $this->colService->getFilesDirectories($objCollection);

        return ($this->apiReply($objDirectories, "", Response::HTTP_OK));
    }
...
//                        break;
        ...
        if (!is_authorized($objUser, "Arena.Office", "Arena.Office.Access", "office") && !$bnSoundblock) {
            return ($this->apiReject("", "You don't have required permissions.", Response::HTTP_FORBIDDEN));
        }
...
//                        break;
        ...
        $bnOffice = is_authorized($objUser, "Arena.Office", "Arena.Office.Access", "office");
...
//                        break;
    ...
    public function confirmFiles(ConfirmMultipleFiles $objRequest) {
        ...
        $bnOffice = is_authorized(Auth::user(), "Arena.Office", "Arena.Office.Access", "office");
...
//            if ($strCategory == Constant::MusicCategory && $objProject->flag_music_permanent) {
//                return ($this->apiReject("", "You can't upload music to the project while it has active deployment.", Response::HTTP_FORBIDDEN));
//            }
        }
        ...
        if (is_array($arrZip)) {
            ...
            $queueJob = $this->colService->processFilesJob($objProject, $objRequest->input("collection_comment"), $objRequest->input("files"), $arrUserMetaInfo);
        }
...
//                        break;
    ...
    public function editFilePath(EditFilePath $objRequest){
        $objCollection = $this->colService->find($objRequest->input("collection"));
        $objUser = Auth::user();
        $strSoundGroup = sprintf("App.Soundblock.Project.%s", $objCollection->project->project_uuid);

        if (!is_authorized($objUser, $strSoundGroup, "App.Soundblock.Project.File.Files.Update", "soundblock")) {
            return ($this->apiReject(null, "You don't have required permissions.", Response::HTTP_FORBIDDEN));
        }

        if (is_null($objCollection)) {
            return $this->apiReject(null, "Collection Not Found", Response::HTTP_BAD_REQUEST);
        }

        if (!$objCollection->files()->where("soundblock_files.file_uuid", $objRequest->input("file_uuid"))->exists()) {
            return ($this->apiReject(null, "File doesn't belongs to this collection.", Response::HTTP_BAD_REQUEST));
        }

        if (!$objCollection->directories()->where("soundblock_files_directories.directory_uuid", $objRequest->input("directory_uuid"))->exists()) {
            return ($this->apiReject(null, "Invalid directory.", Response::HTTP_BAD_REQUEST));
        }

        $objFile = $this->colService->updateFilePath($objRequest->all(["file_uuid", "directory_uuid"]), $objCollection);

        return ($this->apiReply($objFile, "File path was updated successfully.", Response::HTTP_OK));
    }
    ...
    public function updateTrackMeta(UpdateTrack $objRequest){
        ...
        return ($this->apiReply($objTrack, "Track updated successfully.", Response::HTTP_OK));
    }

    /**
     * @param SwapTracks $objRequest
     * @return object
     * @throws \Exception
     */
    public function swapTracks(SwapTracks $objRequest)
    {
        $objUser = Auth::user();
        $objCollection = $this->colService->find($objRequest->input("collection"));
        $strSoundGroup = $this->authGroupService->findByProject($objCollection->project_uuid)->group_name;
        $strPermission = "App.Soundblock.Project.File.Music.Update";
        $bnSoundblock = is_authorized($objUser, $strSoundGroup, $strPermission, "soundblock");
        $bnOffice = is_authorized($objUser, "Arena.Office", "Arena.Office.Access", "office");
        ...
        $objCollection = $this->colService->swapTracks($objRequest->all());

        return ($this->apiReply($objCollection, "Tracks were organized successfully.", Response::HTTP_OK));
    }
     ...
     * @throws \Exception
     */
    public function reorderTracks(ReorderTracks $objRequest)
    {
        $objUser = Auth::user();
        $objCollection = $this->colService->find($objRequest->input("collection"));
        $strSoundGroup = $this->authGroupService->findByProject($objCollection->project_uuid)->group_name;
        ...
        $objCollection = $this->colService->reorderTracks($objRequest->all());
