...
class Artist extends Controller
{
    ...
    public function store(StoreArtist $objRequest){
        ...
        if (
            !is_authorized($objUser, $strSoundGroup, "App.Soundblock.Account.Artist.Create", "soundblock", true, true) ||
        ...
        $arrCreateData= $objRequest->only("artist_name", "url_apple", "url_spotify", "url_soundcloud");
        $arrCreateData['artist_avatar_rand']=mt_rand();
        $objArtist = $this->artistService->create($arrCreateData, $objAccount);

        if ($objRequest->has("project_uuid")) {
            ...
            dispatch(new ProjectLedger(
                $objProject,
                \App\Services\Soundblock\Ledger\ProjectLedger::UPDATE_EVENT,
                [
        ...
        return ($this->apiReply([
            ...
            "account_uuid" => $objArtist->account_uuid,
            'artist_uuid' => $objArtist->artist_uuid,
            "avatar_url" => $objArtist->avatar_url,
            "flag_permanent" => $objArtist->flag_permanent,
            "url_soundcloud" => $objArtist->url_soundcloud,
            "url_spotify" => $objArtist->url_spotify,
            "url_apple" => $objArtist->url_apple,
        ],
        "Artist Stored successfully.",
         Response::HTTP_OK));
    }
    ...
    public function update(UpdateArtist $objRequest){
        ...
        if (
            !is_authorized($objUser, $strSoundGroup, "App.Soundblock.Account.Artist.Update", "soundblock", true, true) ||
            (is_null($objAccount) && is_null($objOwnAccount))
        ) {
            return ($this->apiReject("", "You don't have required permissions.", Response::HTTP_FORBIDDEN));
        }
        ...
        if ($objArtist->flag_permanent) {
            return ($this->apiReject(null, "This artist is part of a deployment and it cannot be updated.", Response::HTTP_FORBIDDEN));
        }
        ...
        if (!empty($objArtistTracks)) {
            foreach ($objArtistTracks as $objTrack) {
                ...
                dispatch(new TrackLedger(
                    $objTrack,
                    \App\Services\Soundblock\Ledger\TrackLedger::UPDATE_EVENT,
                    [
                        "remote_addr" => request()->getClientIp(),
                        "remote_host" => gethostbyaddr(request()->getClientIp()),
                        "remote_agent" => request()->server("HTTP_USER_AGENT")
    ...
    public function delete(DeleteArtist $objRequest){
        ...
        if (
            !is_authorized($objUser, $strSoundGroup, "App.Soundblock.Account.Artist.Delete", "soundblock", true, true) ||
            (is_null($objAccount) && is_null($objOwnAccount))
        ) {
            return ($this->apiReject("", "You don't have required permissions.", Response::HTTP_FORBIDDEN));
        }
        ...
        if((isset($objArtist->projects) && $objArtist->projects->count()) || (isset($objArtist->tracks) && $objArtist->tracks->count())) {
            return ($this->apiReject(null, "This artist is associated with a project or track and it cannot be deleted.", Response::HTTP_FORBIDDEN));
        }

        if ($objArtist->flag_permanent) {
            return ($this->apiReject(null, "This artist is part of a deployment and it cannot be deleted.", Response::HTTP_FORBIDDEN));
        }
