...
class Project extends Controller {
    ...
    public function index(GetProjects $objRequest) {
        if (!is_authorized(AuthFacade::user(), "Arena.Office", "Arena.Office.Access", "office")) {
        ...
        return ($this->apiReply($arrProject, "", Response::HTTP_OK, $availableMetaData));
    }
    ...
    public function show(string $project) {
        ...
        $objProject = $this->projectService->findForOffice($project);
    ...
    public function addMember(AddMember $request, string $project) {
        if (!is_authorized(AuthFacade::user(), "Arena.Office", "Arena.Office.Access")) {
            return $this->apiReject("", "You don't have required permissions.", Response::HTTP_FORBIDDEN);
        }
