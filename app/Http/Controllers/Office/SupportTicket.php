...
class SupportTicket extends Controller {
     ...
     * @throws Exception
     */
    public function index(GetAllSupport $objRequest, AuthGroup $objAuthGroupService) {
        /** @var User $user*/
        $objUser = Auth::user();
        ...
        $intPerPage = $objRequest->get("per_page", 10);
        //-- Check if user has administrative Office Support permissions to view all tickets
        if(is_authorized($objUser, "Arena.Support", "Arena.Office.Access", "office")){
            $objUser = null;
            $arrGroupIds= null;
        }else {
            /** @var \Illuminate\Database\Eloquent\Collection */
            $arrUserGroups = $objAuthGroupService->findByUser($objUser->user_uuid, $intPerPage, false);
            /** @var array */
            $arrGroupIds = $arrUserGroups->pluck("group_id")->toArray();
        }

        [$objTickets, $objAvailableMetaData] = $this->supportTicketService->findAll($objRequest->all(), $intPerPage, $arrGroupIds, $objUser);

        return ($this->apiReply($objTickets, "", Response::HTTP_OK, $objAvailableMetaData));
    }
