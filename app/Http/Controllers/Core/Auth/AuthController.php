...
use App\Http\Requests\Auth\{IsAuthorized,
    SignInAs,
    SignUp,
    Signin,
    RefreshToken,
...
class AuthController extends Controller {
    ...
    public function signInAs(SignInAs $ojbRequest){
        $objUser = Auth::user();
        if (!is_authorized($objUser, "Arena.Support", "Arena.Support.Soundblock.SignInAs", null, true, true)) {
            return ($this->apiReject("", "You don't have required permissions.", Response::HTTP_FORBIDDEN));
        }

        $objUser = $this->userService->find($ojbRequest->input("user_uuid"), true);

        Auth::setUser($objUser);
        $token = $objUser->createToken("bearer")->accessToken;
        $redirectUrl = app_url("soundblock") . "auth/signinas?user={$objUser->user_uuid}&token={$token}";

        $objUser->update(["last_login" => now()]);

        return (
            $this->apiReply(
                ["token" => $token, "url" => $redirectUrl],
                "You are logged in under another user successfully.",
                Response::HTTP_OK
            )
        );
    }
    ...
    public function signUp(SignUp $objRequest) {
        $objApp = Client::app();
        $boolResult = $this->emailService->canCreateEmail($objRequest->input("email"));

        if (!$boolResult) {
            return ($this->apiReject(null, "This email already exist.", Response::HTTP_BAD_REQUEST));
        }
