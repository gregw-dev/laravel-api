...
class ProjectLedger implements ShouldQueue {
     ...
     */
    public function __construct(Project $objProject, string $strEvent, array $arrUserMeta) {
    ...
    public function handle(ProjectLedgerService $projectLedger) {
        ...
        if (!is_null($this->objProject->date_release_pre)) {
            $arrLedgerData["Pre-Release Date"] = $this->objProject->date_release_pre;
        }
        ...
        if (is_null($this->objProject->ledger_id)) {
            ...
            $objLedger = $projectLedger->createDocument($this->objProject, $arrLedgerData, $this->strEvent, $this->arrUserMeta);
