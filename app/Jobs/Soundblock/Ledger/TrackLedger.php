...
class TrackLedger  implements ShouldQueue {
     ...
     */
    public function __construct(TrackModel $objTrack, string $strEvent, array $arrUserMeta, array $arrHistoryChanges = [])
    {
    ...
    public function handle(TrackLedgerService $ledgerService)
    {
        ...
        if (is_null($objLedger)) {
            $arrFileData["Track Metadata"] = [
                ...
                "Track Language ID"           => $this->objTrack->track_language_metadata_uuid,
                "Track Language Vocals ID"    => $this->objTrack->track_language_audio_uuid,
            ...
            $objLedger = $ledgerService->createDocument($this->objTrack, $arrFileData, TrackLedgerService::CREATE_EVENT, $this->arrUserMeta);
