...
class ServiceLedger implements ShouldQueue {
     ...
     */
    public function __construct(Account $objAccount, string $strEvent, array $arrUserMeta) {
    ...
    public function handle(LedgerService $ledgerService) {
        ...
        $objActivePlan = $this->objAccount->activePlan;
        ...
        switch ($this->strEvent) {
            case LedgerService::CREATE_EVENT:
                $arrLedgerData = [
                    ...
                    "Account Active Plan" => isset($objActivePlan) ? $objActivePlan->planType->plan_name : "",
        ...
        if (is_null($this->objAccount->ledger_id)) {
            $ledgerService->createDocument($this->objAccount, $arrLedgerData, LedgerService::CREATE_EVENT, $this->arrUserMeta);
