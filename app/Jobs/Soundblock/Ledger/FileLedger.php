...
class FileLedger implements ShouldQueue {
     ...
     */
    public function __construct(File $objFile, string $strFilePath, array $arrUserMeta) {
    ...
    public function handle(FileLedgerService $ledgerService, FileService $fileService) {
        ...
        if (is_null($objLedger)) {
            ...
            $objLedger = $ledgerService->createDocument($this->objFile, $arrFileData, strtolower($objHistory->file_action), $this->arrUserMeta);
