...
class CollectionLedger implements ShouldQueue {
     ...
     */
    public function __construct(Collection $collection, string $strEvent, array $arrUserMeta, array $filesChanges = []) {
    ...
    public function handle(CollectionLedgerService $collectionLedgerService, FileService $fileService, CollectionService $collectionService) {
        ...
        if (is_null($this->collection->ledger_id)) {
            $objLedger = $collectionLedgerService->createDocument($this->collection, $arrData, CollectionLedgerService::CREATE_EVENT, $this->arrUserMeta);
