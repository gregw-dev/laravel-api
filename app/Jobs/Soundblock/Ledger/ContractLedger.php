...
class ContractLedger implements ShouldQueue {
     ...
     * @throws \Exception
     */
    public function __construct(Contract $objContract, string $strEvent, array $arrUserMeta, ?User $objUser = null) {
    ...
    public function handle(ContractLedgerService $contractLedgerService, ContractService $contractService) {
        ...
        if (is_null($this->objContract->ledger_id)) {
            $objLedger = $contractLedgerService->createDocument($this->objContract, $arrData, $this->strEvent, $this->arrUserMeta);
