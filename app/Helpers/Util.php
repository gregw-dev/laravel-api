...
class Util {
    ...
    public static function draft_artwork(string $url): ?string {
         ...
         * debug
         */
        // $pattern = "#" . URL::to("/storage/soundblock/service") . "/" . $uuid . "/projects/drafts/artwork/" . $uuid . "\.png#";
        $pattern = "#" . env("APP_URL") . "/storage/soundblock/service" . "/" . $uuid . "/projects/drafts/artwork/" . $uuid . "\.png#";
     ...
     * @return string
     */
    public static function draft_artwork_old_path(?string $fileName = null): string {
    ...
    public static function draft_artwork_path(?string $fileName = null): string {
        return is_null($fileName) ? "public" . Constant::Separator . "upload" . Constant::Separator . "artwork" :
            "public" . Constant::Separator . "upload" . Constant::Separator . "artwork" . Constant::Separator . $fileName;
    }
    ...
    public static function draft_artwork_cdn_url(string $fileName): string {
        return (cloud_url("soundblock") . "upload/artwork/" . $fileName);
    }
    ...
    public static function relative_artwork_path($project): string {
        ...
        return "artwork/{$objProject->account->account_uuid}/{$objProject->project_uuid}/{$objProject->project_artwork_rand}.png";
    }
    ...
    public static function make_suffix(array $file, SupportCollection $files, ?int $suffix = 0): array {
        ...
        $result = $files->search(function ($value) use ($file, $suffix) {
            if ($suffix == 0) {
    ...
    public static function getUsersByPermissionAndGroup(string $authGroup, string $authPermission){
        $authGroupService = resolve(AuthGroupService::class);

        return ($authGroupService->findAllUsersByGroupAndPermission($authGroup, $authPermission));
    }

    public static function getIpFraudScore(string $strClientIp): int{
        $strUrl = "https://api11.scamalytics.com/arena/?key=" . env("SCAMALYTICS_KEY") . "&ip=" . $strClientIp;
        ...
        return (intval($response["score"]) ?? 0);
    }

    public static function getUserFraudScore($objUser): array{
        $objRecord = DB::table("oauth_access_tokens")
            ->where("user_id", $objUser->user_id)
            ->latest("created_at")->first();

        return ([
            "remote_addr" => $objRecord->remote_addr ?? $objUser->remote_addr,
            "fraud_score" => $objRecord->fraud_score ?? null
        ]);
    }
}
